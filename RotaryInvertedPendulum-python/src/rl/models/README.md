...
| `policy_working_balance.zip` | Best checkpoint so far: ~97.5% balance over 60s across the 2026-07-11 validation runs, pending re-confirmation on the rig after the soldering rework | `logs/policy_working_balance_1.npz`, `logs/policy_working_balance_2.npz` | `policy_working_balance_replay_buffer.pkl` — to resume fine-tuning with `finetune_async.py --resume-buffer` |
