...
Checkpoints representative of the project's progression, with their
hardware validation log (.npz files, readable via numpy.load) where one
exists. Not the full history of runs/checkpoints.

Four of them trace two independent improvements — frame stacking (fixes
POMDP chattering, see `PLAN/RL.md` Étape 15) and real-rig fine-tuning
(adapts sim training to this rig's actual dynamics) — so where a
checkpoint sits on this 2×2 explains its behavior:

| | Without frame stacking | With frame stacking (3) |
|---|---|---|
| **Before real-rig fine-tuning** | `policy_fails_no_swingup.zip` | `policy_working_balance_before_finetuning.zip` |
| **After real-rig fine-tuning** | `policy_partial_balance.zip` | `policy_working_balance.zip` |
...
| `policy_partial_balance.zip` | Works partially: swings up and balances briefly, then falls, with noticeable chattering — the POMDP problem frame stacking was later built to fix | `logs/policy_partial_balance.npz` | None — the only buffer available for this run corresponds to episode 30's state, not episode 71's; including it would have been misleading |
| `policy_working_balance_before_finetuning.zip` | Balances fine in MuJoCo simulation, never fine-tuned on the rig — see "Pre-fine-tuning comparison point" below | N/A (simulation-only reference point) | None — see below |
